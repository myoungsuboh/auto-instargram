...
- 후속 제약 (중요):
  ...
  - ⚠️ **기존 앱에 추가하는 경로는 문서 설명대로 되지 않는 경우가 있다.**
    ...
    "앱 맞춤 설정 및 요건" — 즉 이미 들어 있는 이용 사례의 완료 체크리스트만 표시되고,
    제품을 더하는 입구는 오른쪽 위의 **"이용 사례 추가"** 버튼이다.
    문서가 말한 "대시보드의 제품 카드"는 **1~5단계로 갓 만든 앱**의 대시보드다.
    따라서 기존 앱 재사용은 신뢰할 수 없고, **새 앱을 만드는 쪽이 확실하다.**
