...
- 기록일: 2026-07-29 16:27 (마지막 갱신 2026-07-30 — [갱신 이력](#갱신-이력) 참조)
- 상태: 승인됨
...
- 긍정: 사용자 안내를 단계별 절차로 모달에 담을 수 있다. 코드와 문서가 같은 경로를 가리켜 어긋나지 않는다.
  (단계 수는 실제로 겪은 함정을 반영하며 바뀐다 — 여기에 숫자를 적어 두면 곧 어긋나므로 적지 않는다.
  현재 내용은 [TokenGuideModal.vue](../../FE/src/components/TokenGuideModal.vue) 가 유일한 출처다.)
...
- 후속 제약 (중요):
  ...
  - 이 프로젝트가 **쓰지 않는** API 설정 화면 항목: Webhooks 구성(웹훅 미사용),
    ...
    앱 검수(본인 계정에만 게시).
  - 장기 토큰은 **`GET /refresh_access_token?grant_type=ig_refresh_token` 으로 60일 연장**할 수 있다
    (조건: 발급 후 24시간 이상 경과, 미만료, `instagram_business_basic` 보유).
    60일 안에 갱신하지 않으면 영구 만료된다. 2026-07-30 확인 —
    그전까지 "60일마다 재발급"만 적어 두어 연장 가능성을 놓치고 있었다.
    자동 연장은 아직 구현하지 않았다([계획 문서](../plans/2026-07-30-per-user-instagram-connect.md) 5단계).
  - 계정 번호는 더 이상 사람이 조회하지 않는다 — 토큰 교환 시 서버가 함께 받아 저장한다
    ([ADR-0024](2026-07-30-fetch-ig-account-on-token-exchange.md)).
  - ⚠️ **"앱 역할 → 역할" 의 Instagram 테스터 초대 경로를 쓰지 말 것 — 폐기된 API 의 장치다.**
    나는 한때 이 경로를 안내했는데(2026-07-30), **잘못이었다.**
    그 역할의 설명이 "Instagram **Basic Display API** 에 필요한 모든 권한…" 이고,
    **Basic Display API 는 2024-12-04 폐기**되어 우리가 쓰는 Instagram Login 방식이 대체했다.
    실제로 초대를 보내면 역할 목록에 "대기 중" 으로 남지만,
    **인스타그램의 "앱 및 웹사이트" 화면에는 "테스터 초대" 탭이 존재하지 않아**
    (활성·만료됨·삭제됨 뿐) 수락할 방법이 없다(2026-07-30 실측).

    계정을 앱에 붙이는 올바른 경로는 **API 설정 화면 → "2. 액세스 토큰 생성" → [계정 추가]** 다.
    문서 7단계: "Instagram 계정을 추가하세요… **Instagram 계정에 로그인하라는 메시지가
    표시됩니다**" — 즉 그 자리에서 인스타그램 로그인으로 붙이며, 별도 초대·수락 절차가 없다.
    이때 **브라우저가 연결할 계정으로 로그인돼 있어야** 하고, 다른 계정으로 로그인된 상태면
    "개발자 역할 권한이 부족합니다" 가 난다.

    (혼동의 원인: API 설정 화면 2번 항목에 Meta 가 "역할 탭에서 Instagram 테스터 역할이
    할당되었는지 확인하세요" 라는 <b>낡은 문구</b>를 여전히 표시한다. 그 문구를 따라가면
    막힌다.)
  ...
  - ⚠️ **위를 모두 갖춰도 같은 오류가 나는 Meta 쪽 미해결 문제가 있다.**
    ...
    2026-07-30 확인 시 **상태가 "해결되지 않음"** 이다
    ([스레드](https://developers.facebook.com/community/threads/1360157352263976/)).
    우리 코드로 우회할 수 없다 — 토큰 발급 자체가 Meta 콘솔에서 이뤄지기 때문이다.
    이 제약은 사용자별 계정 연결([계획 문서](../plans/2026-07-30-per-user-instagram-connect.md))
    의 실현 가능성에도 영향을 준다: 같은 인증 단계를 쓰므로 OAuth 로 바꿔도 막힐 수 있다.
...
결정("Instagram Login 경로를 쓴다")은 처음부터 바뀌지 않았다. 아래는 **그 결정에 따르는
제약을 실제로 해 보며 채워 넣은 기록**이다. 세 번 모두 내가 문서를 잘못 읽거나 문서 문구를
검증 없이 옮겨 적은 것이 원인이었으므로, 같은 실수를 반복하지 않도록 남긴다.

| 시점 | 무엇이 틀렸나 | 어떻게 바로잡았나 |
|---|---|---|
| 2026-07-29 | 앱 생성 시 **이용 사례·앱 유형** 조건이 안내에 아예 없었다. 페이스북 페이지가 필요하다고 잘못 적었다 | 공식 문서 재확인 → 후속 제약 3건과 출처 2건 추가 |
| 2026-07-30 (1차) | 문서 문구 "기존 앱에 추가하려면 6단계부터"를 검증 없이 옮겼는데, **기존 앱 대시보드에는 제품 카드가 없었다** | 실측 후 정정. 다만 이때 "기존 앱은 안 되니 새 앱을 만들라"고 **과도하게 일반화** |
| 2026-07-30 (2차) | 위 일반화가 틀렸다 — **"이용 사례 추가" 목록에 Instagram 이용 사례가 있었다** | 정확한 경로(추가 버튼 → 이용 사례 이름)로 교체 |
| 2026-07-30 (3차) | `client_secret` 을 Facebook 앱 시크릿으로 안내. `instagram_business_content_publish` 가 **자동으로 붙는다**고 적었다 | API 설정 화면 실측 → 둘 다 정정. 각각 토큰 교환·게시 실패로 직결되는 항목 |
