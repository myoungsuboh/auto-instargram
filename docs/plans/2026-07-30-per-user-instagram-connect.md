...
| **선행 확인 — 지금 방식으로 토큰이 한 번이라도 발급되는가** | 🚧 **막힘** — "개발자 역할 권한이 부족합니다". 원인으로 의심했던 2건(테스터 초대 미수락 / 계정 유형)은 **오판으로 확인됐다.** 남은 미시도: 연결할 계정으로 로그인된 브라우저에서 [계정 추가] 실행(시크릿 창). 상세는 아래 위험. **이것이 안 되면 이 계획은 시작 근거가 없다** |
...
- ⚠️ **가장 큰 위험 — 인증 단계 자체가 Meta 쪽에서 막혀 있을 수 있다 (2026-07-30 발견).**
  ...
  **다만 아직 Meta 버그로 단정할 수 없다** — 같은 날 확인 결과, 막힘의 원인으로
  ...
  → 아직 시도하지 않은 것: **브라우저가 연결할 인스타그램 계정으로 로그인된 상태에서**
  [계정 추가] 를 실행하는 것(시크릿 창 권장). 다른 계정으로 로그인돼 있으면 같은 오류가 난다.
  이것까지 실패하면 Meta 버그 쪽으로 무게가 실린다.
