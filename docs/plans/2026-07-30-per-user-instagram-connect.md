...
| **선행 확인 — 지금 방식으로 토큰이 한 번이라도 발급되는가** | 🚧 **막힘** — "개발자 역할 권한이 부족합니다". 의심했던 4건(계정 유형 / 테스터 초대 미수락 / 게시 권한 미추가 / 브라우저 로그인 계정)은 **모두 배제됐다.** 남은 유력 변수는 **비즈니스 포트폴리오 연결 + 인증 검토중**. 다음 실험은 **포트폴리오에 연결하지 않은 새 앱**으로 변수 제거(아래 위험 참조). **이것이 안 되면 이 계획은 시작 근거가 없다** |
...
- ⚠️ **가장 큰 위험 — 인증 단계 자체가 Meta 쪽에서 막혀 있을 수 있다 (2026-07-30 발견).**
  ...
  (Insufficient developer role) 로 막혔다. 비즈니스 관리자 권한·앱 역할·페이지 연결이
  모두 정상인데도 같은 오류가 난다는 신고가 Meta 개발자 커뮤니티에 있고,
  확인 시점에 **상태가 "해결되지 않음"** 이다
  ([스레드](https://developers.facebook.com/community/threads/1360157352263976/)).

  **다만 아직 Meta 버그로 단정할 수 없다** — 같은 날 확인 결과, 막힘의 원인으로
  의심했던 두 가지가 오판이었다:
  1. **"Instagram 테스터 초대 미수락"이 원인이라고 봤는데 아니었다.** 그 역할은 폐기된
     Instagram Basic Display API(2024-12-04 폐기)의 장치이고, 인스타그램에 수락 화면이
     아예 없다(활성·만료됨·삭제됨 탭뿐). 올바른 경로는 **API 설정 → "2. 액세스 토큰 생성"
     → [계정 추가]** 에서 그 자리에서 인스타그램 로그인으로 붙이는 것이다
     (상세: [ADR-0022](../decisions/2026-07-29-instagram-login-api-path.md) 후속 제약).
  2. **계정 유형은 문제가 아니었다** — 프로페셔널 계정임을 확인했다.

  3. **브라우저 로그인 계정도 원인이 아니었다** — 시크릿 창에서 연결할 계정으로 로그인해
     [계정 추가] 를 실행했으나 **같은 오류**였다(2026-07-30). 이 가설은 배제됐다.
     (같이 배제된 것: 게시 권한 미추가 — Standard Access 는 자동 승인이다.)

  → **남은 유력 변수: 비즈니스 포트폴리오.** 이 앱은 비즈니스 포트폴리오
  `Harness-system` 에 연결돼 있고("이 앱은 Harness-system에서 관리합니다" +
  [비즈니스 관리자에서 역할 수정] 버튼), 그 비즈니스의 **인증이 "검토 중"** 이다.
  `앱 역할` 문서 원문: "앱이 비즈니스 포트폴리오에 연결된 경우 **비즈니스 관리자를 사용하여
  앱 역할을 관리해야 합니다**"
  ([출처](https://developers.facebook.com/docs/development/build-and-test/app-roles)).
  즉 개발자 역할이 앱 대시보드에서 해석되지 않을 수 있다. 커뮤니티 스레드의 신고자도
  비즈니스 관리자가 얽힌 상황이었다 — 공통점이다.

  → **다음 실험(변수 제거)**: **비즈니스 포트폴리오에 연결하지 않은 새 앱**을 만들어
  거기서 [계정 추가] 를 시도한다. 앱 생성 시 비즈니스 연결은 선택이다(문서: "지금 또는
  개발 과정에서 언제든 비즈니스를 연결할 수 있습니다"). 이 한 번으로 비즈니스 포트폴리오·
  인증 검토중·광고/Threads 이용 사례 혼재가 동시에 제거된다. 되돌릴 수 없는 조작은 없고,
  실패하면 새 앱을 지우면 된다.
  - 되면 → 원인은 비즈니스 포트폴리오/인증. 기존 앱은 인증 완료 후 재시도하거나 새 앱을 쓴다.
  - 안 되면 → Meta 버그로 거의 확정. 그때는 이 계획의 착수 근거가 사라지므로
    사용자와 방향을 다시 정해야 한다.

  **이 계획에 직접 영향이 있다**: 아래 2단계(OAuth) 는 같은 인증 창을 거친다.
  즉 HTTPS 를 해결하고 OAuth 를 다 구현해도 **같은 지점에서 막힐 수 있다.**
  → **0단계보다 먼저** 확인해야 할 것: 지금 방식(앱 대시보드 [계정 추가] → [토큰 생성])으로
