...
**서버(백엔드) 창이 아래처럼 멈춰 보이는 것은 정상입니다.**

```
[############...] 80% EXECUTING [12m 34s]
```

이 명령은 서버를 **붙잡고 실행하는 방식**이라, 서버가 살아 있는 동안 계속 그 상태로 시간만
올라갑니다. 진행이 안 되는 게 아닙니다 — 오히려 이 표시가 사라지면 서버가 꺼진 것입니다.

서버가 정말 떴는지는 그 창에서 이 줄을 찾으면 됩니다:

```
Started BackendApplication in 19.013 seconds
